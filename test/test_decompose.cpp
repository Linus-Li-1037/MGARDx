...
void test(string filename, const vector<size_t>& dims, int target_level){
    ...
    // MGARD::writefile("/pscratch/xli281_uksr/wli/SCALE_d64/data/decomposed.dat", data.data(), num_elements);
    test_recompose(data, dims, target_level);
    // std::cout << std::setprecision(std::numeric_limits<double>::max_digits10) << "pos 66449, ori:" << data_ori[66449] << ", dec:" << data[66449] << std::endl;
